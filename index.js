...
const addNoInteractiveOption = (command) =>
  command.option("--no-interactive", "Disable interactive prompts.");

const getCommandOptions = (commandOrOptions) => {
...
commander
  ...
  .action(runAction(async () => {
    await logCurrentConfig({ json: commander.opts().json });
  }));

const listCommand = addNoInteractiveOption(commander
  .command("list")
  .alias("ls")
  .option("--json", "Print machine-readable JSON.")
  ...
  .action(runAction(async () => {
    await listAccounts(undefined, {
      json: listCommand.opts().json || commander.opts().json,
    });
  })));

addNoInteractiveOption(commander
  .command("add")
  .description("Add an account.")
  .argument("[flag]", "Account Flag")
  .argument("[username]", "Account Username")
  .argument("[email]", "Account Email")
  .option("--force", "Overwrite an existing account flag.")
  .action(runAction(async (flag, username, email, command) => {
    ...
    await addAccount(flag, username, email, { force: options.force });
  })));
...
addNoInteractiveOption(commander
  ...
  .option("--force", "Overwrite an existing account flag.")
  .action(runAction(async (flag, command) => {
    const options = getCommandOptions(command);
...
addNoInteractiveOption(commander
  ...
  .option("--yes", "Confirm global config changes.")
  .action(runAction(async (flag, command) => {
    const { global, yes } = getCommandOptions(command);
    ...
    if (!flag) {
      ...
      return selectAnAccount(obj, global, { yes });
    }

    validateFlag(flag);
    const account = obj.accounts[flag];
    if (account) {
      await useAnAccount(flag, account, global, { yes });
    } else {
      console.log(
...
addNoInteractiveOption(commander
  ...
  .action(runAction(async (flag, command) => {
    const { all } = getCommandOptions(command);
    if (all) {
      await clearFile();
